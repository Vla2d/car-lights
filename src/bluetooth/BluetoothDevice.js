...
export async function isDeviceSupported(device) {
  ...
  const serialService = services.find((it) => it.uuid.startsWith(BT05_SERVICE_UUID_PREFIX));
  const characteristics = await serialService?.characteristics();
  const serialCharacteristic = characteristics.find(
    (it) => it.uuid.startsWith(BT05_CHARACTERISTIC_UUID_PREFIX),
  );
  return serialCharacteristic != null;
}

export async function sendCommandTo(device, command) {
